    ...
    _runSpec: function(spec) {
        ...
        var horseman = new Horseman({phantomPath: phantomPath})
            ...
            .on('loadFinished', function() {
                ...
                run.call(horseman);
                horseman.waitFor(function() {
                    var images = document.getElementsByTagName('img');
                    return Array.prototype.every.call(images, function(i) { return i.complete; });
                }, true);
                if (spec.cropArea) {
                    horseman.crop(spec.cropArea, spec.output);
                } else {
                    horseman.screenshot(spec.output)
                }
                horseman.close()
                    .then(function() {
                        this.runningInstances--;
            ...
            .on('timeout', function() {
                console.warn(spec.url);
                console.warn('TIMEOUT: waiting 3 seconds before proceeding');
                horseman.wait(3000);
            })
            .open(spec.url);
    }
